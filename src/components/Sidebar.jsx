    ...
    return (
        <div className="glass-panel fade-in" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            ...
            {session ? (
                ...
                /* Secure Lock State when User is not logged in! */
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px', textAlign: 'center', gap: '16px' }}>
                    <div style={{ background: 'rgba(255,255,255,0.05)', padding: '20px', borderRadius: '50%', border: '1px solid var(--panel-border)' }}>
                        <Lock size={32} color="var(--text-muted)" />
                    </div>
                    <h2 style={{ fontSize: '1.25rem', color: 'var(--text-main)', margin: 0, fontWeight: '600' }}>Authentication Required</h2>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem', lineHeight: '1.5', margin: 0 }}>
                        Please <strong>Log In</strong> or <strong>Sign Up</strong> using the Navbar above to access live global flight schedules and plotting tools.
                    </p>
                </div>
            )}
        </div>
